...
console.log("Bookshelf app script loaded.");
...
function toggleView() {
    ...
    viewToggleButton.textContent = `View: ${currentView === 'grid' ? 'List' : 'Grid'}`;
...
async function startScanner() {
    ...
    // Ensure ZXing library is loaded (it should be from CDN)
    if (typeof ZXing === 'undefined') {
        console.error("ZXing library not loaded!");
        scanStatusElement.textContent = 'Error: Scanner library failed to load.';
        return;
    }
    ...
    try {
        // Ask for camera permissions and get stream
        // Use facingMode: 'environment' to prefer rear camera
        const constraints = {
            video: {
        ...
        scanStatusElement.textContent = 'Starting scanner...';
        console.log('Camera stream obtained');

        // Start decoding from the video element
        codeReader.decodeFromStream(currentCameraStream, videoScannerElement, (result, err) => {
            if (result) {
                // Barcode found!
                console.log('Barcode detected:', result);
                // Check if it might be an ISBN (EAN-13 usually starts with 978 or 979)
                const barcodeText = result.getText();
                if (result.getBarcodeFormat() === ZXing.BarcodeFormat.EAN_13 && (barcodeText.startsWith('978') || barcodeText.startsWith('979'))) {
                    console.log(`Detected ISBN: ${barcodeText}`);
                    scanStatusElement.textContent = `ISBN Found: ${barcodeText}`; // Update status
                    stopScanner(); // Stop scanner immediately
                    // Automatically trigger the lookup
                    handleIsbnLookup(barcodeText); // Pass detected ISBN to lookup function
                } else {
                    // Optional: Provide feedback if a non-ISBN barcode is scanned
                     console.log(`Scanned non-ISBN code: ${barcodeText}, Format: ${ZXing.BarcodeFormat[result.getBarcodeFormat()]}`);
                     // scanStatusElement.textContent = `Scanned ${ZXing.BarcodeFormat[result.getBarcodeFormat()]}. Looking for ISBN...`;
                }
            }

            if (err && !(err instanceof ZXing.NotFoundException)) {
                 // Log errors other than simply not finding a barcode
                console.error('ZXing decoding error:', err);
                scanStatusElement.textContent = 'Scanner error.';
            }
        });

        console.log(`ZXing scanner started for device: ${currentCameraStream.getVideoTracks()[0]?.label}`);
        ...
        // Optionally show the manual start button if auto-start fails
        // if (startScanButton) startScanButton.style.display = 'block';
        stopScanner(); // Ensure cleanup if start failed
    }
}
...
function stopScanner() {
    ...
    if (currentCameraStream) {
        currentCameraStream.getTracks().forEach(track => track.stop()); // Stop all tracks
        currentCameraStream = null;
        console.log("Camera stream stopped.");
    }
    if (videoScannerElement) {
        videoScannerElement.srcObject = null; // Clear video source
        videoScannerElement.pause();
        videoScannerElement.load(); // Attempt to reset video element state
    }
     if (scanStatusElement) {
        // Reset status text when stopped manually or on close
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Use global constants directly here
    if (!bookDisplayArea) {
        console.error("Essential UI element #book-display-area not found. Aborting initialization.");
        return;
    }
    if (!addBookButton) {
        console.warn("#add-book-btn not found.");
    }
    if (!isbnInputContainer || !isbnManualInput || !isbnLookupButton || !cancelIsbnInputButton) {
        console.warn("ISBN input modal elements not found. ISBN lookup disabled.");
    }
    // Re-check the add book form elements using global constants
    if (!addBookFormContainer || !addBookForm || !cancelAddBookButton || !addBookFormTitle) {
        // This warning might still appear if elements genuinely aren't ready, but let's ensure consistency
        console.warn("Add book form elements might not be found. Add functionality may be broken.");
    }
    if (!resetDataButton) {
        console.warn("#reset-data-btn not found.");
    }
...
// Other functions (Search, Filter etc. later) 